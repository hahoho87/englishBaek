						...
						<c:forEach items="${list}" var="payment">
							...
							<tr>
								...
								<td><c:if test="${payment.paymentState !='결제취소완료'}">
										<a class="move" id="cancel" href="${payment.paymentNo}">${payment.paymentState}
										</a>
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<c:forEach items="${list2}" var="paymentRefundNo">
							<tr>
								...
								<td><fmt:formatDate value="${paymentRefundNo.paymentDate}"
										pattern="yyyy-MM-dd" /></td>
			...
			.on(
					...
					function(e) {
						...
						if (deletcheck == true) {
							...
							operForm
									.append("<input type='hidden' name='paymentNo' value='"
											+ $(this).attr('href') + "'>'");
							operForm.attr("action", "/payment/modify").attr(
									"method", "post");
							e.preventDefault();
							operForm.submit();
						} else {
