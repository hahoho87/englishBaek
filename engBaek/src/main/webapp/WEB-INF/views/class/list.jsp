    ...
    <div class="row">
      <div class="col-md-12 col-lg-8">
        <div class="title-single-box">
          ...
          <span class="color-text-a">${classDataList[0].courseName }</span>
        </div>
      </div>
  ...
  <div class="container">
    <div class="row">
        ...
        <tbody>
          <c:forEach items="${classDataList}" var="classData">
            ...
            <tr>
              ...
              <td><a class="move" href="${classData.classDataNo}"> ${classData.classDataTitle}</a></td>
              <td>${classData.name}</td>
              <td><fmt:formatDate value="${classData.classDataRegdate}" pattern="yyyy-MM-dd" /></td>
            </tr>
          </c:forEach>
        </tbody>
      </table>
      ...
      <div class="col-xs-4 col-md-12 pull-right">
        ...
        <button id="classListBtn" type="button" class="btn btn-a pull-right">강의목록</button>
        <button id="regBtn" type="button" class="btn btn-a pull-right" style="margin-right:10px">Register</button>
      </div>
      <!-- END 페이지 번호 출력 -->
    </div>
  </div>
...
<script>
	$(function() {
		...
		$(".move").on("click", function(e) {
			e.preventDefault();
			actionForm.append("<input type='hidden' name='classDataNo' value='" + $(this).attr('href') + "'>'");
