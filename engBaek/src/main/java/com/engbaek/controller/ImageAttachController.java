...
public class ImageAttachController {
	...
	@ResponseBody
	public ResponseEntity<ImageAttachDTO> 
			uploadAjaxPost(MultipartFile[] uploadFile) {
		ImageAttachDTO imageDTO = new ImageAttachDTO();
		...
		for (MultipartFile m : uploadFile) {
			...
			imageDTO.setTeacherProfilePicture(uploadFileName);	//1.업로드 파일명 저장
			...
			try {
				...
				imageDTO.setTeacherProfileUuid(uuid.toString());  //2.UUID 값 저장
