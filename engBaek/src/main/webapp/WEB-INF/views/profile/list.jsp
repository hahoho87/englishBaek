    ...
    <div class='row'>
      <div class="col-lg-6">
        <form id="searchForm" action="/profile/list">
          <select name="type" class="form-control form-control-lg form-a" style="width: 100px;">
          ...
          <input type="text" class="form-control form-control-lg form-control-a" placeholder="검색"
                  name="keyword" value="${pageMaker.cri.keyword }" style="width: 200px"> 
	...
	$(function() {
		...
		$(function() {
			...
			$('#search')
					.change(
							function() {
								searchValue = $("#search option:selected")
										.val();
								var actionForm = $('#mainForm');
								console.log(searchValue);

								if (searchValue == "ALL") {
									actionForm.attr("action", "/profile/list")
											.submit();
									return false;
								}
								actionForm
										.append("<input type='hidden' name='type' value='C'>");
								actionForm
										.append("<input type='hidden' name='keyword' value='"+searchValue+"'>")
								actionForm.attr("action", "/profile/list")
										.submit();
								return false;
							});
				...
				function(e) {
					...
					if (operation === 'remove') { //삭제 버튼이 눌린 경우 
						...
						if (deleteConfirm == true) {
							alert("삭제가 완료되었습니다.")
							var teacherPnoTag = $("input[name='teacherPno']")
									.clone();
							formObj.append(courseCodeTag);

							formObj.attr("action", "/profile/remove").attr(
									"method", "post");
						} else {
							return;
						}
						...
						formObj.attr("action", "/profile/list").attr("method",
								"get");
						...
						formObj.attr("action", "/profile/modify").attr(
								"method", "get");
